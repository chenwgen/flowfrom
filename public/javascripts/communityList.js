...
document.write('\
	<style type="text/css">\
		#divAddressMenu {line-height:20px;text-align:left;position:absolute;visibility:hidden;z-index:10000;;overflow:hidden;width:282px;background:#fff;border:solid #B8E1FF 1px;border-top:gray solid 1px;font-size:12px;}\
		...
		#divAddressMenu a {display:block;width:272px !important;width:100%;padding:1px 2px 2px 2px;cursor:default;text-decoration:none;color:#3298CB;background-color:none;}\
		#divAddressMenu a span{float:right;font-family:vernada;display: none }\
		#divAddressMenu a:hover {border:solid #B8E1FF 1px;background-color:#EFF8FF;}\
	    .sel {border:solid 1px #B8E1FF;background-color:#EFF8FF }\
	</style>\
');
var PageId=0,PageNum;
var varAddressListString="";
function GetCityList(fctThisObj){
    ...
    var varAddressList=varAddressListString;
	//alert(varAddressList);
   // varAddressList =varAddressList +'xuanhuaqu|宣化|XH|1758|1@hengdianzhen|横店|XH|2593|1@';
	...
	with(varMenuObj){
		...
		innerHTML="<div><h4 style='font-family:vernada'>输入中文/拼音或→←↑↓选择</h4>"+(varHtmlStr==""?"<nobr>‘"+varObjValue+"’没有相应小区信息。</nobr>":varHtmlStr+varNextPageStr)+"</div>";
	}
...
function setCityListStyle(index,lks){
    for(var i=0;i<lks.length;i++) {lks[i].className = '';lks[i].style.color = '#3298CB'; }
    lks[index].className = 'sel';
}
